...
def test(device, rank, local_rank):
    ...
    d_id = device.index
    print(f"the local_rank is {local_rank}, the device is {d_id}, the transfer time is:{gen_time}, the bandwith is :{bandwith} MB/s")
    y = x1 * x2
    return y
